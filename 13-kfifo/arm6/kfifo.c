...
{
    /* 这里采用硬件提供的计算前导零指令 CLZ
     * 举个例子，假如变量data_roundup_pow_of_two 0x09
     *（二进制为：0000 0000 0000 0000 0000 0000 0000 1001）, 即bit3和bit0为1
     * 则__clz( (data_roundup_pow_of_two)的值为28,即最高位1 前面有28个0,32-28 =3 代表最高位1 的 位置
     * 31UL 表示 无符号 int 数字 31，否则默认为 有符号 int 数字 31
     */

    return ( 1UL << ( 32UL - ( unsigned int ) __clz( (size) -1) ) );
}
                   ...
                   unsigned int esize)
{
	...
	fifo->data =(uint8_t *) pvPortMalloc(size * esize);
...
#define	KFIFO_PEEK(data, out, mask) \
	((data)[(out) & (mask)])
/*
 * __kfifo_peek_n internal helper function for determinate the length of
 * the next record in the fifo
 */
static unsigned int kfifo_peek_n(struct __kfifo *fifo,  unsigned int recsize)
{
    unsigned int l;
    unsigned int mask = fifo->mask;
    unsigned char *data = fifo->data;

    l = KFIFO_PEEK(data, fifo->out, mask);

    if (--recsize)
        l |= KFIFO_PEEK(data, fifo->out + 1, mask) << 8;
...
{
    return kfifo_peek_n(fifo, recsize);
}
